...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
            "SELECT c FROM Comment c WHERE c.articleId IN (SELECT a.id FROM Article a WHERE a.categoryId = :categoryId)"
                    + " AND (:includeDeleted = true OR c.deletedAt IS NULL) ORDER BY c.createdAt DESC")
    ...
    @Query(
            "SELECT c FROM Comment c WHERE (:includeDeleted = true OR c.deletedAt IS NULL) ORDER BY c.createdAt DESC")
